...
const SPARKLES_COUNT = 5;
const UPDATE_INTERVAL = 100;
const LIFESPAN_DECREASE = 0.1;

const SparkleIcon = ({ id, x, y, color, delay, scale }: Omit<Sparkle, 'lifespan'>) => (
    <motion.svg
        key={id}
...
const SparkleEffect = () => {
    ...
    const generateStar = useCallback((): Sparkle => {
        ...
        const color = Math.random() > 0.5 ? 'fill-pink-500' : 'fill-violet-500';
        ...
        const updateStars = () => {
            setSparkles((currentSparkles) =>
                currentSparkles.map((star) =>
                    star.lifespan <= 0 ? generateStar() : { ...star, lifespan: star.lifespan - LIFESPAN_DECREASE },
                ),
            );
        };
